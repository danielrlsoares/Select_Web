...
if ($isAuth){
	if (isset($_POST['nome']) && isset($_POST['telefone'])){
		...
		$result = pg_query($con, "UPDATE usuario SET nome_usuario = '$nome', telefone_usuario = '$telefone' WHERE email_usuario = '$username'");
		...
		$response["success"] = 0;
		$response["error"] = "Parâmetros não foram enviados";
	}
}
else {
	$response["success"] = 0;
